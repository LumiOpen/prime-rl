        ...
        with open(dataset_path) as f:
            for line in f:
                ...
                if isinstance(gt, list):
                    gt = gt[0] if gt else ""
                # Drop samples where the EN and FI versions have different turn counts —
                # this indicates the role markers were mistranslated or dropped.
                if use_finnish:
                    en_turns = len(_parse_prompt(row.get("prompt", "") or ""))
                    fi_turns = len(_parse_prompt(prompt))
                    if en_turns != fi_turns:
                        continue
                rows.append({
                    "prompt": prompt,
                    "ground_truth": gt,
...
def _parse_prompt(prompt_text: str) -> list[dict]:
    ...
    Handles both single-turn ("user: ...") and multi-turn dialogs where
    turns are delimited by "user:", "assistant:", "käyttäjä:", "avustaja:".
    """
...
def _build_example(row: dict) -> dict | None:
    ...
    messages = _parse_prompt(str(prompt_text))
    judge_context = _format_dialog_for_judge(messages)
